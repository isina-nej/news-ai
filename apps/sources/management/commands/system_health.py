...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 2. Redis Cache
        try:
            cache.set("healthcheck:probe", "1", timeout=5)
            redis_status = "OK" if cache.get("healthcheck:probe") == "1" else "FAILED"
        except Exception as exc:
            redis_status = f"FAILED: {exc}"
        self.stdout.write(f"Redis Cache: {redis_status}")
        ...
        self.stdout.write(
            f"Telegram Ingestion (Kurigram): API ID Configured={tg_id_set}, "
            f"Session Configured={tg_session_set}"
        )
        ...
        self.stdout.write(
            f"Telegram Bot Publishing: Token Configured={bot_token_set}, "
            f"Channel Configured={channel_set}, Auto-Publish={auto_pub}"
        )
        ...
        self.stdout.write(
            f"Twitter/X Ingestion: Session Configured={twitter_session_set}, "
            f"Enabled={twitter_enabled}"
        )
